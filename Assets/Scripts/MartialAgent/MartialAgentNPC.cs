...
public class MartialAgentNPC : Agent
{
    /*
        <기초 에이전트 스펙:>
        최대 체력 = 13
    ...
    // Win Message Invoker
    public delegate void WinMessageRaisedEventHandler();
    public static event WinMessageRaisedEventHandler OnWinMessageRaised;
    ...
    public override void Initialize()
    {
        ...
        Spd의 스피드, 탄두 인식 범위 증가량: x/5
         */
        // Player는 여기서 데이터를 로드하나, NPC는 외부에서 입력된 값대로 반영한다.
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        */
        ...
        if (collision.TryGetComponent<Bullet>(out Bullet bullet) && (!isInvincible))
        {
            ...
            float targetRangeDamage = targetTransform.GetComponent<MartialAgentPlayer>().rangeDamage;
    ...
    private void HitByMelee()
    {
        // HitByMelee
        IsHit = false;
        float targetMeleeDamage = targetTransform.GetComponent<MartialAgentPlayer>().meleeDamage;
